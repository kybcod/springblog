        ...
        <div class="col-4">
            ...
            <table class="table">
                ...
                <c:forEach var="search" items="${searchTitle}" varStatus="status">
                    ...
                    <tr onclick="location.href='${link}'">
                        <td style="color: blue">${titlePageInfo.totalBoard - (titlePageInfo.currentPage - 1) * 10 - status.index}</td>
                        <td>${search.title}</td>
                        <td>${search.writer}</td>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center">
                    ...
                    <c:if test="${titlePageInfo.currentPage > 1}">
                        <li class="page-item">
                    ...
                    <c:if test="${titlePageInfo.prevPage > 0}">
                        <li class="page-item">
                            <c:url value="board/search/" var="prevPageLink">
                                <c:param name="page" value="${titlePageInfo.prevPage}"/>
                            </c:url>
                    ...
                    <c:forEach begin="${titlePageInfo.beginPage}" end="${titlePageInfo.endPage}" var="page">
                        <c:url value="/" var="pageLink">
                            <c:param name="page" value="${page}"/>
                        </c:url>
                        <li class="page-item ${titlePageInfo.currentPage eq page ? 'active' : ''}">
                            <a class="page-link" href="${pageLink}">${page}</a>
                        </li>
                    </c:forEach>

                    <c:if test="${titlePageInfo.nextPage < titlePageInfo.lastPage}">
                        <li class="page-item">
                            <c:url value="/" var="nextPageNumber">
                                <c:param name="page" value="${titlePageInfo.nextPage}"/>
                            </c:url>
                    ...
                    <c:if test="${titlePageInfo.currentPage < titlePageInfo.lastPage}">
                        <li class="page-item">
                            <c:url value="/" var="lastPageLink">
                                <c:param name="page" value="${titlePageInfo.lastPage}"/>
                            </c:url>
